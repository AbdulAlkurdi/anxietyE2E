...
def paths_with_results_generator(architecture, dataset, eval_i, fold_i, folds_n, setups):
    for setup in setups:
        yield f"/mnt/d/Users/alkurdi/anxietyE2E/results_archive/{dataset}_{folds_n}fold_{fold_i:02d}/tune_{eval_i:02d}/{architecture}/{setup}"
def count_classes_representation():
    counts = {}
    results = []
    for dataset in ['WESAD']: # "ASCERTAIN", "DECAF", "Amigos", ""]:
        counts[dataset] = []
        for subject in range(100):
            path = f"/mnt/c/Users/alkurdi/Desktop/Vansh/data/E2Epreprocessed/{dataset}/y_{subject}.pkl"
